...
# Groq Settings
GROQ_API_KEY = (os.environ.get("GROQ_API_KEY") or os.environ.get("groq") or "").strip()
...
"""
                    contents = [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": quiz_prompt}]
                except Exception as e:
                    logger.error(f"Error parsing quiz action: {e}")
                    contents = [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": user_message}]
            else:
            ...
            async with httpx.AsyncClient(timeout=15.0) as http_client:
                async with http_client.stream("POST", groq_url, headers={"Authorization": f"Bearer {GROQ_API_KEY}"}, json=payload) as response:
                    if response.status_code >= 400:
                        error_text = await response.aread()
                        error_msg = error_text.decode('utf-8', errors='ignore')
                        error_lower = error_msg.lower()
                        # FULL error log karo taaki debug ho sake
                        logger.error(f"[Groq Error] Status: {response.status_code} | Body: {error_msg[:500]}")
                        ...
                        # Model not found check (specific to model errors)
                            ...
                            raise ValueError(f"Groq API error {response.status_code}: {error_msg[:200]}")
