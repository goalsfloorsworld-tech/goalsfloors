...
export default function BrandMarquee() {
  const brands = [
    ...
    { name: "Darpan Puri", src: "/brand logo/DARPAN_PURI-removebg-preview.png", scale: 1.4 },
    ...
    { name: "Vikash Builders", src: "/brand logo/vikash-builders-removebg-preview.png", scale: 1.4 },
    ...
    <section className="py-10 bg-gray-50 border-t border-gray-100 overflow-hidden">
      ...
      <div className="relative w-full overflow-hidden bg-white py-12 border-y border-gray-100">
        ...
        <div className="animate-marquee flex items-center gap-12 md:gap-24 px-12">
          {scrollingBrands.map((brand: { name: string; src: string; scale?: number }, i) => (
            <div 
              key={i} 
              className="flex-shrink-0 w-32 h-16 md:w-48 md:h-24 relative opacity-90 hover:opacity-100 transition-all duration-300 cursor-pointer"
            >
              <Image 
                src={brand.src} 
                alt={brand.name} 
