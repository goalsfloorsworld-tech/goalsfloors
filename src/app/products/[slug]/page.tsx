...
export default function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 font-sans antialiased transition-colors duration-300">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            {[
              ...
              <div key={i} className="flex flex-col items-start hover:-translate-y-1 transition-transform cursor-default">
                <div className="w-12 h-12 bg-white dark:bg-slate-950 rounded-lg flex items-center justify-center mb-5 shadow-sm border border-gray-100 dark:border-gray-800 text-amber-600 dark:text-amber-500 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-slate-950 py-10 lg:py-16 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
            
            <div className="w-full lg:w-1/2">
              <h2 className="inline-block text-sm font-semibold uppercase tracking-widest text-amber-600 mb-4 px-3 py-1 bg-amber-50 dark:bg-amber-900/20 rounded-sm">
                Product Details
              </h2>
              ...
              <ul className="space-y-4">
                {product.features.map((feature, idx) => (
                  <li key={idx} className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-900 transition-colors">
                    <CheckCircle2 className="w-5 h-5 text-amber-600 dark:text-amber-500 flex-shrink-0 mt-0.5" />
                    <span className="text-base text-gray-800 dark:text-gray-300 font-medium">{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="w-full lg:w-1/2 grid grid-cols-2 gap-6">
              {product.images.slice(1,3).map((img, i) => (
                 <div key={i} className={`relative aspect-[4/5] bg-gray-100 dark:bg-slate-900 rounded-lg overflow-hidden border border-gray-100 dark:border-gray-800 shadow-sm ${i === 1 ? 'mt-4' : ''}`}>
          ...
          <div className="space-y-4">
            {product.faqs.map((faq, i) => (
              ...
              >
                ...
                >
                  <h4 className={`text-lg font-medium pr-8 transition-colors duration-300 ${openFaqIndex === i ? 'text-amber-600 dark:text-amber-500' : 'text-gray-900 dark:text-white'}`}>
                    {faq.question}
                  </h4>
