...
const categoryConfig: { [key: string]: { label: string; icon: any } } = {
  ...
  "premium-flooring": { label: "Premium Flooring", icon: Maximize },
...
function ProductsContent() {
  ...
  const dataCategories = Array.from(new Set(productsData.map(p => p?.category).filter(Boolean)));
  ...
  // Robust Search Logic
  const searchResults = searchQuery.length > 0 ? productsData.filter(product => {
  ...
  const filteredProducts = categoryParam === "All" 
    ? productsData 
    : productsData.filter(p => p?.category === categoryParam);
  ...
  return (
    <div className="bg-[#fcfaf7] dark:bg-slate-950 min-h-screen pb-24 transition-colors duration-500">
      ...
      <section className={`bg-gray-950 dark:bg-black pt-20 pb-20 md:pt-32 md:pb-48 border-b border-amber-600/30 relative transition-[z-index] ${isResultsVisible ? 'z-[100]' : 'z-10'}`}>
        <div className="absolute inset-0 opacity-20 pointer-events-none overflow-hidden">
            <div className="absolute top-0 right-0 w-96 h-96 bg-amber-600 blur-[150px] rounded-full translate-x-1/2 -translate-y-1/2"></div>
            <div className="absolute bottom-0 left-0 w-96 h-96 bg-blue-600 blur-[150px] rounded-full -translate-x-1/2 translate-y-1/2"></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-[110] text-center">
          <motion.h1 
            ...
            className="text-2xl md:text-5xl font-bold text-white tracking-tight mb-12 uppercase italic"
          >
            Browse Our <span className="text-amber-500">Full</span> Collection.
          </motion.h1>
        ...
        <div className="max-w-7xl mx-auto px-4">
            ...
            <div className="hidden md:flex justify-center">
                <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl p-2 rounded-full shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-stone-200 dark:border-slate-800 flex items-center gap-2"
                >
                    {categories.map((cat) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-3 px-6 py-3.5 rounded-full transition-all duration-500 shrink-0 relative group ${
                                    isActive 
                                        ? "text-white" 
      ...
      {/* Products Display Grid - Lower Z-index (z-10) */}
