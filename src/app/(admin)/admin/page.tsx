...
export default async function AdminDashboard() {
  ...
  try {
    ...
    if (msg.includes('DECODER routines')) {
      msg = 'Invalid GOOGLE_PRIVATE_KEY format in .env.local (OpenSSL key decode failed). Please make sure GOOGLE_PRIVATE_KEY is wrapped in double quotes (") and copied exactly from your Google Service Account JSON file.';
    }
    errorMsg = msg;
    hasData = false;
  }
      ...
      {!hasData ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-gray-300 dark:border-slate-800 bg-white/60 dark:bg-slate-950/40 backdrop-blur p-12 shadow-sm text-center">
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400 max-w-md mt-2">
            Could not fetch live report data from Google Analytics. Please verify that your credentials (`GA_PROPERTY_ID`, `GOOGLE_CLIENT_EMAIL`, `GOOGLE_PRIVATE_KEY`) in `.env.local` are set correctly.
          </p>
          {errorMsg && (
